...
def evaluate(args, model, eval_dataset, ignore_index, global_step=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            #----
            # sample_perplexity = torch.exp(torch.tensor(lm_loss.mean().item()))
            # for i, sid in enumerate(sample_ids):
            #     sample_results[sid] = {
            #         'loss': lm_loss[i].item(),
            #         'perplexity': sample_perplexity[i].item()
            #     }
            #----
    ...
    #----
    # output_sample_results_file = os.path.join(eval_output_dir, "sample_eval_results.json")
    # with open(output_sample_results_file, "w") as f:
...
def main():
    ...
    #----
    train_data = GPT21024Dataset(args.root_dir,args.ids_file,mode='train',length=28711) #training on only 3000 datasets
    valid_data = GPT21024Dataset(args.root_dir,args.ids_file,mode='valid',length=1149)  #validation on only 500 datasets
